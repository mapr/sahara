...
class Oozie(s.Service):
    def __init__(self):
        ...
        self.ui_name = 'Oozie'
        self.version = '4.0.1'
        self.node_processes = [OOZIE]
        self.dependencies = [
            ('mapr-oozie-internal', self.version),
        ]
        self.cluster_defaults = ['oozie-default.json']
        self._validation_rules = [vu.exactly(1, OOZIE)]

    def get_config_files(self, cluster_context, configs, instance=None):
        oozie_conf_path = '/opt/mapr/oozie/oozie-%s/conf/' % self.version
        oozie_site = bcf.HadoopXML("oozie-site.xml")
        oozie_site.remote_path = oozie_conf_path
        if instance:
    ...
    def post_install(self, cluster_context, instances):
        ...
        if oozie_service:
            ...
            with oozie_inst.remote() as r:
                LOG.debug('Copying MySQL connector to Oozie lib dir')
                r.execute_command(symlink_cmd, run_as_root=True,
                                  raise_when_error=False)

    def post_start(self, cluster_context, instances):
        home_dir = "/opt/mapr/oozie/oozie-%s" % self.version
        ...
        with oozie_inst.remote() as r:
            LOG.debug("Copying Oozie sharelibs")
