...
class MapReduce(s.Service):
    ...
    def __init__(self):
        ...
        self.ui_name = 'MapReduce'
        # FIXME: little misconception
        self.name = 'mapreduce'
        self.version = '0.20.2'
        self.node_processes = [JOB_TRACKER, TASK_TRACKER]
        self.ui_info = [
    ...
    def _get_packages(self, node_processes):
        ...
        result += [(np.get_package(), None) for np in node_processes]

        return result

    # mapred-site.xml
    def get_config_files(self, cluster_context, configs, instance=None):
        hadoop_conf = "/opt/mapr/hadoop/hadoop-%s/conf/" % self.version

        core_site = bcf.HadoopXML("core-site.xml")
        core_site.remote_path = hadoop_conf
        if instance:
        ...
        mapred_site.remote_path = hadoop_conf
        if instance:
    ...
    def _get_mapred_site_props(self, context):
        ...
        result = dict()
        if context.get_instances(hue.HUE):
            result = {
                'jobtracker.thrift.address': '0.0.0.0:9290',
                'mapred.jobtracker.plugins': 'org.apache.hadoop.thriftfs'
