...
HBASE_THRIFT = np.NodeProcess(
    ...
    package='mapr-hbasethrift',  # FIXME: Name for maprcli
    open_ports=[9090]
)
...
class HBase(s.Service):
    def __init__(self):
        ...
        self.name = 'hbase'
        self.ui_name = 'HBase'
        self.node_processes = [HBASE_MASTER, HBASE_REGION_SERVER, HBASE_THRIFT]
        self.cluster_defaults = ['hbase-default.json']
    ...
    def get_config_files(self, cluster_context, configs, instance=None):
        remote_path = '/opt/mapr/hbase/hbase-%s/conf/' % self.version
        hbase_site = bcf.HadoopXML("hbase-site.xml")
        hbase_site.remote_path = remote_path
        if instance:
...
class HBaseV094(HBase):
    def __init__(self):
        ...
        self.version = '0.94.24'
        self.dependencies = [('mapr-hbase', self.version)]
