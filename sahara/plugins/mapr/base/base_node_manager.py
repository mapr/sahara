...
import oslo.utils.timeutils as timeutils
...
class BaseNodeManager(s.AbstractNodeManager):
    def move_nodes(self, cluster_context, instances):
        LOG.info(_LI("START: Moving the nodes to the /decommissioned"))
        ...
        LOG.info(_LI("END: Moving the nodes to the /decommissioned"))
    ...
    def remove_nodes(self, c_context, instances):
        ...
        with random.choice(cldb_instances).remote() as cldb_remote:
            for instance in instances:
                ...
                cldb_remote.execute_command(command, run_as_root=True)
        LOG.info("END: removing the nodes")
    ...
    def _start_zk_nodes(self, instances):
        LOG.info(_LI('START: Starting ZooKeeper nodes'))
        self._start_nodes(instances, mng.ZOOKEEPER.get_ui_name())
        LOG.info(_LI('END: Starting ZooKeeper nodes'))

    def _start_cldb_nodes(self, instances):
        LOG.info(_LI('START: Starting CLDB nodes'))
        self._start_nodes(instances, WARDEN_SERVICE)
        LOG.info(_LI('END: Starting CLDB nodes'))

    def _start_non_cldb_nodes(self, instances):
        LOG.info(_LI('START: Starting non CLDB nodes'))
        self._start_nodes(instances, WARDEN_SERVICE)
        LOG.info(_LI('END: Starting non CLDB nodes'))

    def _stop_zk_nodes(self, instances):
        self._stop_nodes(instances, mng.ZOOKEEPER.get_ui_name())
    ...
    def await_no_heartbeat():
        ...
        LOG.info(_LI('START: Waiting %ss for "NO_HEARBEAT" alarm' % delay))
        context.sleep(delay)
        LOG.info(_LI('END: Waiting %ss for "NO_HEARBEAT" alarm' % delay))
