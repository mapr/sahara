...
class Context(bc.BaseClusterContext):
    UBUNTU_MAPR_BASE_REPO = ('http://package.mapr.com/releases/v4.0.2/ubuntu/ '
                             'mapr optional')
    UBUNTU_MAPR_ECOSYSTEM_REPO = ('http://package.mapr.com/releases/'
                                  'ecosystem-4.x/ubuntu binary/')
    CENTOS_MAPR_BASE_REPO = 'http://package.mapr.com/releases/v4.0.2/redhat/'
    ...
    def get_install_repo_script_data(self):
        template = f.get_file_text('plugins/mapr/resources/add_mapr_repo.sh')
        return template % {
